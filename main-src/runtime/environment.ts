...
export function createGlobalEnv(beginTime: number = -1, filePath: string = __dirname, args: RuntimeVal[] = [], currency: string = "-"): Environment {
    ...
    env.declareVar("import", MK_NATIVE_FN((args) => {
        ...
        let input;
        if(path.endsWith(".lim")) {
            input = fs.readFileSync(path, "utf-8");
        } else if (path.endsWith(".limx")) {
            if(currency == "-") throw "Cannot run LIM X from LIM: " + path;
            input = transcribe(fs.readFileSync(path, "utf-8"), currency);
        } else throw "Not a LIM [X] file: " + path
    ...
    function closeLIM(): null {
        if(beginTime != -1) {
            console.log(`\nLIM executed in ${(Date.now() - beginTime).toLocaleString()}ms.`);
        }
        process.exit();
    }
